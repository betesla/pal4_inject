...
launcher 的“增强功能 -> 松散文件补丁”提供一个统一勾选开关，默认启用并使用 `替换（可回退）`。它同时控制 CPK 资源和 CS 文本脚本。取消勾选后恢复原版加载路径；由于发行版没有 `editData`，关闭后 CS 模式通常不能工作。高级页仍可选择完整 HookMode，但两种替换模式对 CS 都执行“仅 gamepatch、无回退”策略。
