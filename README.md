...
- 发布启动入口：
  ...
  - “控制”页支持 Xbox 360 / XInput 手柄按钮映射；默认 A 键统一镜像空格键的按下与释放，同时同步 Win32 键盘路径和 PAL4 原生 InputManager 状态，可用于主菜单、读档、对话和场景交互；现代控制模式使用左摇杆连续方向，大角度改向会以行走动作平滑转身，并在跑到快跑区间同步线性调节移动与动作速度；右摇杆调整视角，并可实验性保持自由镜头模式
