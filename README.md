...
- 发布启动入口：
  ...
  - 当前内置版本为 `v0.2.5`，发布 Release 时建议使用同名 tag；构建号只用于定位具体构建时间
...
原游戏在 CS 模式中把 `Music` 和 `worldMap` 的文本脚本误写成了 `.csb` 请求；注入层会在文本解释器入口自动改查 `Music.cs` / `worldMap.cs`。不要把未编译源码保存为 `.csb`：二进制加载入口只接受长度头合法的编译后 CSB，否则忽略该覆盖并回退原 CPK。
...
脚本默认从 `CMakeLists.txt` 读取版本号，例如 `0.2.5` 会生成 tag/release 版本 `v0.2.5`。产物只有一个含 `PAL4.exe` 的完整安装包 `PAL4Plus_v0.2.5_win32.zip`，供手动安装与自动更新共用；另在 `build/update-release` 生成配套 `update.json`。两个发布源均先上传 ZIP、最后上传清单。如只想本地打包、不发布：
...
```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\release.ps1 -ReleaseNotesPath .\docs\release_notes_v0.2.5.md
```
...
- 对共享 `CEGUI_Renderer_Constructor_2` 路径上的宽屏分辨率（例如 `1600x900`、`1680x1050`、`1920x1080`）：
  ...
  - 标题主界面 `MainWindow/Root`、“旧的回忆”读档页、“前尘忆梦”影片列表、图片列表/查看器、旧版在线信息页、Loading，以及游戏内人物/物品/装备/仙术/锻造/任务/系统菜单使用宽屏布局，不再绘制左右黑边；游戏内菜单只铺宽公共连接条并把左右外框贴边，中央人物、列表、存档和弹窗保持原始尺寸；Loading 只铺宽背景并保持进度组件居中；帮助、制作人员、世界地图等复合贴图界面仍保持 4:3
- 游戏内常驻 HUD 会额外补一层“贴边”修正；项目早期手改的 `ui_ab` 仅作为历史效果参考，不视为官方宽屏资源：
  - `minimap.xml` 可见部件改为靠左下
  - `portrait.xml` 可见部件改为靠右上
  - 标题主界面的左右装饰和按钮分别靠边，中央 Logo 保持居中，只横向扩展上下连接条；背景继续独立铺满宽屏
  - 标题流程原本绘制的 `PAL4 v1.1` 会在内存中改为 `PAL V1.2.5`，便于从截图直接识别正在使用本修补；补丁先校验原始 12 字节数据槽，不修改磁盘上的 `PAL4.exe`
  ...
  - “前尘忆梦”播放页的左右竖框贴近屏幕边缘，上下横框只沿水平方向拉伸，右下按钮框随右边框移动；视频画面本身不拉伸
  - 战斗页将敌人数贴左，展开底部状态栏并重新分布三名角色状态卡；行动条与由战斗逻辑动态定位的小头像作为一个整体保留原位，避免静态条框和动态头像错位。行动盘、物品/仙术/特技选择窗和战斗结算按各自结构独立定位，结算四块人物属性面板继续由原版入场动画展开，战斗场景和中央内容不拉伸
