...
- 发布启动入口：
  ...
  - 当前内置版本为 `v0.1.7`，发布 Release 时建议使用同名 tag；构建号只用于定位具体构建时间
...
脚本默认从 `CMakeLists.txt` 读取版本号，例如 `0.1.7` 会生成 tag/release 版本 `v0.1.7`，产物为 `PAL4_inject_v0.1.7_win32.zip`。如只想本地打包、不发布 GitHub Release：
